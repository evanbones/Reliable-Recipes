...
public class TagModifier {
    ...
    private static <T> List<HolderSet.Named<T>> getAllTags(Registry<T> registry) {
        //? if <1.21.2 {
        /*return registry.getTags().map(pair -> pair.getSecond()).toList();
        *///?} else {
        return registry.getTags().toList();
        //?}
    }

    private static <T> Optional<HolderSet.Named<T>> getTag(Registry<T> registry, TagKey<T> key) {
        //? if <1.21.2 {
    ...
    private static <T> void applyToRegistry(Registry<T> registry, String debugName) {
        ...
        if (!clearTagRules.isEmpty()) {
            for (HolderSet.Named<T> tag : getAllTags(registry)) {
                Identifier tagId = tag.key().location();
                for (TagRule rule : clearTagRules) {
                    if (rule.tagMatcher().test(tagId)) {
                        int size = tag.size();
                        if (size > 0) {
                            Constants.LOG.info("TagModifier: Clearing tag '{}' (contained {} entries)", tagId, size);
                            removalCount += size;
                        }
                        clearTag(tag);
                        break;
            ...
            for (T object : registry) {
                ...
                if (shouldRemoveAll) {
                    for (TagKey<T> key : tags) {
                        getTag(registry, key).ifPresent(tag ->
    ...
    private static <T> int removeHiddenValuesFromTags(Registry<T> registry, Set<T> hiddenValues) {
        ...
        for (T hiddenValue : hiddenValues) {
            ...
            for (TagKey<T> tagKey : holder.tags().toList()) {
                ...
                getTag(registry, tagKey).ifPresent(tag ->
