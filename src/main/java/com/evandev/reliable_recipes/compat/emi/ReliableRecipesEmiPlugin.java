...
public class ReliableRecipesEmiPlugin implements EmiPlugin {
            ...
            try {
                ...
                if (!inputStacks.isEmpty()) {
                    for (EmiStack inputStack : inputStacks) {
                        Identifier id = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "/brewing/" + BuiltInRegistries.ITEM.getKey(output.getItemStack().getItem()).getPath() + "_" + Math.abs(brewingRecipe.hashCode()));
    ...
    private void registerCustomAnvilRepairRecipes(EmiRegistry registry) {
        for (Item item : BuiltInRegistries.ITEM) {
            ...
            if (customMat != null && !customMat.isEmpty()) {
                Identifier id = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "/anvil_repair/" + BuiltInRegistries.ITEM.getKey(item).getPath());
    ...
    private boolean matchesUserRemovalRule(EmiRecipe recipe) {
        ...
        Identifier recipeId = recipe.getId() != null ? recipe.getId() : Identifier.withDefaultNamespace("emi_recipe");
    ...
    private JsonObject createJsonFromEmiRecipe(EmiRecipe recipe) {
        ...
        Identifier categoryId = recipe.getCategory() != null ? recipe.getCategory().getId() : null;
        if (categoryId != null) {
    ...
    private boolean isRepairRecipe(EmiRecipe recipe) {
        ...
        if (recipe.getCategory() != null) {
            Identifier categoryId = recipe.getCategory().getId();
            if (categoryId != null) {
