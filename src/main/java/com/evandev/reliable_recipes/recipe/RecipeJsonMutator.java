        ...
        if (element.isJsonObject()) {
            ...
            for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
                ...
                if (val.isJsonPrimitive() && val.getAsJsonPrimitive().isString()) {
                    ...
                    boolean hasMatch = activeReps.containsKey(strVal) || (nextContext && (activeReps.containsKey("") || activeReps.containsKey("*")));
                    if (hasMatch) {
                        JsonElement repSource = activeReps.containsKey(strVal) ? activeReps.get(strVal)
                                : (activeReps.containsKey("*") ? activeReps.get("*") : activeReps.get(""));
                        JsonElement replacement = repSource.deepCopy();
            ...
            for (int i = 0; i < arr.size(); i++) {
                ...
                if (val.isJsonPrimitive() && val.getAsJsonPrimitive().isString()) {
                    String strVal = val.getAsString();
                    boolean hasMatch = activeReps.containsKey(strVal) || (isOutputContext && (activeReps.containsKey("") || activeReps.containsKey("*")));
                    if (hasMatch) {
                        JsonElement repSource = activeReps.containsKey(strVal) ? activeReps.get(strVal)
                                : (activeReps.containsKey("*") ? activeReps.get("*") : activeReps.get(""));
                        JsonElement replacement = repSource.deepCopy();
