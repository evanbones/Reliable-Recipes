...
The configuration file allows you to strip tags from items using specific tag arrays.
This is extremely useful for cleaning up recipe viewers (JEI/EMI) or removing unobtainable items from tag groups.

## Action: `remove_all_tags` (or `remove_tag`)
...
Completely empties a tag of all its associated items.
