...
Unlike other recipe actions, `prevent_repair` does not require a `filter` object.
Instead, it directly targets items based on the provided item.
...
If you want to disable the vanilla crafting grid repair feature for **all** items while still allowing Anvil or Grindstone repairs, you can use the standard `remove_recipes` action instead:
